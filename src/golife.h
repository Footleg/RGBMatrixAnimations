 ...
 *
 * Based on code which was originally published in my Ardunio code examples
 * repo: https://github.com/Footleg/arduino
 *
...
class GameOfLife {
 ...
 private:
  ...
  uint8_t**
      cells;  // 8bits representing
              // [colour3,colour2,colour1,prev3,prev2,prev1,birth/death,alive]
  uint16_t alive = 0;
  uint16_t population[popHistorySize] = {};
  uint8_t popCursor = popHistorySize -
                      1;  // Set to last position as gets incremented before use
  ...
  // functions
 public:
  GameOfLife(RGBMatrixRenderer&, uint8_t, uint16_t, uint8_t, uint8_t = 1,
             uint8_t = 1);
  ~GameOfLife();
  void runCycle();
  void restart();
  ...
  RGB_colour getCellColour(uint16_t x, uint16_t y);
