 ...
 *
 * This abstract class provides a template for building renderer classes to
 * control LED Matrix displays. It provides methods to set the colour of any
 * LED/pixel in a grid, and to keep the position of points within the bounds of
 * the grid (with or without wrapping over the edges). A series of animation
 * classes use the interface defined in this class to output to displays. By
 * writing a renderer class for any LED matrix hardware (or on screen display)
 * this allows the animations classes to be used with many different hardware
 * display devices without requiring code changes in the animation class.
 *
 * This class also implements a wrapping mode for panels arranged as faces of a
 * cube, including tracking the x and y components of acceleration in the plane
 * of each face of the cube for a 3D acceleration vector. The enables animations
 * to update the motion of pixels on any panel based on the cube face they
 * appear on, and wrapping of pixels across panel edges in cube mode.
 *
...
// default constructor
RGBMatrixRenderer::RGBMatrixRenderer(uint16_t width, uint16_t height,
                                     uint8_t brightnessLimit, bool inCubeMode)
    : gridWidth(width),
      gridHeight(height),
      maxBrightness(brightnessLimit),
      cubeMode(inCubeMode) {
  ...
  if (inCubeMode) {
    ...
    if (width == height * 3 / 2) {
      ...
      throw std::invalid_argument(
          "Cube mode only supports arrangements of 3 x 2 panels.");
    }
  }
...
uint16_t RGBMatrixRenderer::getGridWidth() { return gridWidth; }

uint16_t RGBMatrixRenderer::getGridHeight() { return gridHeight; }

uint8_t RGBMatrixRenderer::getMaxBrightness() { return maxBrightness; }

RGB_colour RGBMatrixRenderer::getRandomColour() {
  // Fetches a random colour from the palette if palette is full, otherwise
  // returns a new one
...
RGB_colour RGBMatrixRenderer::newRandomColour() {
  ...
  if (colour.r < minBrightness && colour.g < minBrightness &&
      colour.b < minBrightness) {
  ...
  // This method for string concatenation was used as it compiles on both g++ on
  // Linux and arduino platforms
  /*
      char msg[32];
      sprintf(msg, "New RGB colour  %d, %d, %d\n", colour.r,  colour.g,
     colour.b); outputMessage(msg);
  */
  return colour;
}

// Method for a normal rectangular flat grid, updates a grid coordinate while
// keeping on the matrix, with optional wrapping
uint16_t RGBMatrixRenderer::newPosition(uint16_t position, uint16_t increment,
                                        uint16_t dimension, bool wrap) {
  int16_t newPos = position + increment;

  // char msg[64];
  // sprintf(msg, "NewPos: Value  %d; Increment: %d; Limit: %d\n", position,
  // increment, dimension); outputMessage(msg);

  if (wrap) {
    while (newPos < 0) newPos += dimension;
    while (newPos >= dimension) newPos -= dimension;
  } else {
    if (newPos < 0)
      newPos = 0;
...
// Method for a cube, updates a grid coordinates while keeping on the matrix,
...
MovingPixel RGBMatrixRenderer::updatePosition(MovingPixel pixel, bool wrap) {
  ...
  if (fineInc >= SUBPIXEL_RES) {
    ...
    // wholePix, partPix, pixel.fineX, pixel.fineY, newPixel.fineX,
    // newPixel.fineY); outputMessage(msg1);
  } else if (pixel.vx < 0) {
    newPixel.x = pixel.x;
  ...
  if (cubeMode) {
    // For cube, the panels are arranged in 2 rows of 3. The bottom row of 3
    // panels represent 3 sides with x being across, and y being up. The top row
    // represents the top, back and bottom sides of the cube. x and y directions
    // vary across these 3 panels wrt to the underlying 3 x 2 matrix.
    ...
    // Determine which panel the new pixel is on (before cube wrapping
    // translation is applied)
    uint8_t panelRowNew = newPixel.y / panelSize;
    uint8_t panelColNew = newPixel.x / panelSize;

    // char msg1[96];
    // sprintf(msg1, "Cube Mode. Wrap=%d On Panel: %d,%d Moving to panel: %d,%d
    // pos: %d,%d ", wrap, panelCol, panelRow, panelColNew, panelRowNew,
    // pixel.x, pixel.y); outputMessage(msg1);
    ...
    if (wrap == false) {
      ...
      if (panelCol == panelColNew) {
        ...
        // Process operation
        if (translate == 0) {
          // Flip X and Y within panel, then translate to position of final
          // panel
          newPixel.x = panelSize - 1 - (newPixel.x - panelColNew * panelSize) +
                       panelColNewCube * panelSize;
          newPixel.y = panelSize - 1 - (newPixel.y - panelRowNew * panelSize) +
                       panelRowNewCube * panelSize;
          ...
          // Swap X and Y within panel, flip Y, then translate to position of
          // final panel
          newX = panelSize - 1 - (newPixel.y - panelRowNew * panelSize) +
                 panelColNewCube * panelSize;
          newPixel.y = (newPixel.x - panelColNew * panelSize) +
                       panelRowNewCube * panelSize;
          newPixel.x = newX;
          // Transpose part pixel positions
          newFineX = -newPixel.fineY;
          ...
          // Translate x to new panel, Y wraps as for non-cube arrangement of
          // flat panels, so no change in Y
...
uint16_t RGBMatrixRenderer::newPositionX(uint16_t x, uint16_t increment,
                                         bool wrap) {
  return newPosition(x, increment, gridWidth, wrap);
}

uint16_t RGBMatrixRenderer::newPositionY(uint16_t y, uint16_t increment,
                                         bool wrap) {
  return newPosition(y, increment, gridHeight, wrap);
}

RGB_colour RGBMatrixRenderer::blendColour(RGB_colour start, RGB_colour end,
...
uint16_t RGBMatrixRenderer::getColourId(RGB_colour colour) {
  ...
  if ((colour.r != 0) || (colour.g != 0) || (colour.b != 0)) {
    for (uint16_t i = 1; i <= coloursDefined; i++) {
      ...
      if ((palette[i].r == colour.r) && (palette[i].g == colour.g) &&
        ...
        // Determine if closest match so far (only needed if palette is already
        // full)
        uint16_t score = abs(palette[i].r - colour.r) +
                         abs(palette[i].g - colour.g) +
                         abs(palette[i].b - colour.b);
    ...
    if (id == 0) {
      if (coloursDefined < maxColours - 1) {
        ...
        // colour.g, colour.b, coloursDefined); outputMessage(msg2);
        ...
        sprintf(msg3, "Asked for (%d,%d,%d) but got (%d,%d,%d)\n", colour.r,
                colour.g, colour.b, palette[id].r, palette[id].g,
                palette[id].b);
        outputMessage(msg3);
      }
    }
...
uint16_t RGBMatrixRenderer::getPixelValue(uint16_t index) { return img[index]; }
...
RGB_colour RGBMatrixRenderer::getPixelColour(uint16_t x, uint16_t y) {
  return getColour(img[y * gridWidth + x]);
}

// Sets pixel colour in memory only (will not show changes until update display
// called) when persistent, else set instant
void RGBMatrixRenderer::setPixelColour(uint16_t x, uint16_t y,
                                       RGB_colour colour, bool persistent) {
  if (persistent) {
...
// Sets pixel colour directly on display. Faster and non-persistent as in memory
// display buffer does not get updated with the change
void RGBMatrixRenderer::setPixelInstant(uint16_t x, uint16_t y,
                                        RGB_colour colour) {
  setPixel(x, y, colour);
}

void RGBMatrixRenderer::drawOctants(int xc, int yc, int x, int y, int yPrev,
                                    RGB_colour colour, bool solid,
                                    bool persistent) {
  if (x > 0) {
    if (y == yPrev || solid == false) {
      ...
      // Inner most pixels of horizontal section, so fill all postions in x
      // range
...
void RGBMatrixRenderer::drawCircle(int xc, int yc, int radius,
                                   RGB_colour colour, bool solid,
                                   bool persistent) {
  int r = radius;
  int x = 0, y = r, yPrev = y;
  int d = 3 - 2 * r;
  // Draw the 4 pixels marking the top, bottom, left and right most points in
  // the circle
  drawOctants(xc, yc, x, y, yPrev, colour, solid, persistent);
  // Now draw the positions either side of the last points drawn and continue
  // moving around the circumference drawing 8 pixels each step
  while (y >= x) {
    x++;
