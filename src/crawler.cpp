 ...
 *
 * Generates a simple animation starting at a random point in the grid, and
 * crawling across the grid at random. This is a simple example of a reusable
 * animator class where the RGB matrix hardware rendering class is passed in, so
 * it can be used with any RGB array display by writing an implementation of a
 * renderer class to set pixels/LED colours on the hardware.
 *
...
Crawler::Crawler(RGBMatrixRenderer &renderer_, uint16_t steps,
                 uint16_t minSteps, bool anyAngle)
    : renderer(renderer_),
      leadPixel(0, 0, 0, 0),
      colChgCount(steps),
      dirChgCount(minSteps),
      anyAngle(anyAngle) {
...
void Crawler::runCycle() {
  // Example of how to output messages to console, compatible with Arduino and
  // C++ on Linux
  /*
      char msg[44];
      sprintf(msg, "%s %d %s %d %s %d %s", "Drawing colour: ", colour.r, ",",
  ...
  */
  ...
  if (dirChg > dirChgCount) {
    ...
    if (dir != 0) {
      if (leadPixel.vx == 0) {
        if (anyAngle) {
          leadPixel.vx =
              dir * rand() % renderer.SUBPIXEL_RES + renderer.SUBPIXEL_RES;
  ...
  // char msg[64];
  // sprintf(msg, "New pos: %d,%d Vel: %d,%d\n", leadPixel.x, leadPixel.y,
  // leadPixel.vx, leadPixel.vy ); renderer.outputMessage(msg);
